...
class AnalysisTransCircleGraphViewHolder(
    ...
    private val defaultColorList = listOf(
        getResColor(little.goose.common.R.color.red_700),
        getResColor(little.goose.common.R.color.red_500),
        getResColor(little.goose.common.R.color.red_200)
    )
    ...
    fun bindData(list: List<little.goose.account.data.models.TransactionPercent>) {
        if (!isInit) {
            initChart(list)
            isInit = true
        } else {
    ...
    private fun initChart(list: List<little.goose.account.data.models.TransactionPercent>) {
        listTransform(list)
        listColors.getColorList(entries.size)
        pieDataSet.apply {
            valueTextSize = 14F
            valueTextColor = getResColor(little.goose.common.R.color.white)
            colors = listColors
        }
        ...
        binding.pieChart.apply {
            ...
            setHoleColor(getResColor(little.goose.common.R.color.primary_color))
            setCenterTextColor(getResColor(little.goose.common.R.color.nor_text_color))
            setCenterTextSize(14F)
            description = null //去掉图表描述
            legend.isEnabled = false //隐藏标注
            setOnChartValueSelectedListener(object : OnChartValueSelectedListener {
                override fun onValueSelected(entry: Entry?, highLight: Highlight?) {
                    val transPercent = entry?.data as? little.goose.account.data.models.TransactionPercent
                        ?: return
    ...
    private fun updateData(list: List<little.goose.account.data.models.TransactionPercent>) {
        listTransform(list)
        listColors.getColorList(entries.size)
        binding.pieChart.apply {
    ...
    private fun listTransform(list: List<little.goose.account.data.models.TransactionPercent>) {
        entries.clear()
